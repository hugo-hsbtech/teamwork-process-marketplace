...
|---|---|---|---|
| v1 | 2026-06-03 | Intake drafted | Registro criado a partir da especificação fonte e sessão de perguntas com o Submitter (Q001–Q008). Blocking sections: problem e reach acima do limiar; originator parked (conf 60); impact abaixo do limiar (conf 30, open). |
| v2 | 2026-06-03 | Intake updated (Rev 2) | Impact atualizado com modelo de dois camadas (Q009 + Q010, conf 70, assumption + discovery flag). Originator atualizado para origem coletiva/bottom-up (Q011, conf 85, answered). Reach enriquecido com segmentos de mercado comercial (Q008/Q010, conf 78). Assumptions (f)–(h) adicionadas (Q005 + Q010). Readiness recomputado: score 63 %, gate CLEARED (todos os quatro bloqueantes ≥ limiar). Triage recomputado; Discovery brief atualizado. |
...
| **Open dispositions** | 8 assumptions a validar (a)–(h) · 1 discovery flag (validação comercial/mercado em impact) · urgency ausente (não-bloqueante) |

> **Nota de cálculo (Rev 2):** Fórmula: média simples dos 5 campos de captura gradeados = (problem + originator + reach + impact + urgency) / 5.
...
> - urgency: 0 / min 70 — abaixo, porém `blocks=false`; não bloqueia o portão
>
> Score: (84 + 85 + 78 + 70 + 0) / 5 = 317 / 5 = **63 %**
>
> **Portão:** CLEARED — todos os quatro campos bloqueantes (problem, originator, reach, impact) atendem ou superam seu `min-confidence`. Urgência permanece sem resposta (não-bloqueante); impacto comercial/mercado é flag de discovery explícita, não gap de captura. DRAFT — pendente confirmação humana.
...
A principal dor não é a votação em si, mas a **falta de estrutura e rastreabilidade no processo de estimativa de tarefas em equipes ágeis**. As sessões de Planning Poker acontecem de forma improvisada, com ferramentas não projetadas para isso.

**Sintomas observáveis recorrentes:**
- Perda de tempo organizando a dinâmica da votação (operação manual em vez de facilitação).
- Falta de registro das estimativas: sem histórico consolidado das sessões.
- Dificuldade em acompanhar a evolução do consenso dentro e entre sessões.
- Ausência de histórico para entender por que uma tarefa recebeu certa estimativa — equipes precisam repetir discussões porque a sessão anterior não ficou registrada.
- Votos influenciados involuntariamente: quando a votação não é verdadeiramente secreta, participantes influenciam os demais antes da revelação.
- Fragmentação entre várias ferramentas (chamadas de vídeo, chats, planilhas, soluções de Planning Poker sem integração), gerando retrabalho e perda de contexto.

**Workaround atual e o que ele não entrega:** o processo é conduzido com chamadas de vídeo + chats corporativos + planilhas para registrar resultados + soluções gratuitas de Planning Poker desconectadas do restante do processo. Essas abordagens resolvem só a votação pontual, não o fluxo completo. Ficam faltando: histórico consolidado das sessões, registro das rodadas por tarefa, justificativas para divergências, exportação de resultados, organização das tarefas estimadas, integração com ferramentas de gestão de produto e visibilidade da evolução das estimativas ao longo do tempo.

**Síntese:** a estimativa, que deveria ser rápida e colaborativa, vira uma atividade operacionalmente desgastante, pouco rastreável e difícil de reproduzir — não por falta de método, mas por falta de uma ferramenta adequada para conduzi-lo.

`Confidence:` 84 · `Source:` Submitter direct (Q006 + Q007) · `Status:` answered · `Disposition:` answered · `Hint:` Dor descrita com sintomas observáveis e sem prescrever solução — atende o rubric. Ainda não quantificada (tempo perdido por sessão, número de sessões): quantificação pertence a `impact` (Q003, open).
...
Dessas dores recorrentes surgiu, entre os próprios participantes, a discussão sobre a necessidade de uma ferramenta mais adequada para suportar o Planning Poker de forma estruturada. Esse reconhecimento coletivo do problema motivou conceber o PokerPlan — depois **formalizado como aposta de produto comercial (SaaS)**, dado que o mesmo problema é amplamente distribuído entre equipes de tecnologia ágil (não exclusivo de um time ou empresa).

**Canal/situação:** concepção interna de produto, a partir de dor observada coletivamente. Não há um sponsor individual nomeado — coerente com a origem bottom-up, não é uma lacuna. Pode ser confirmado se necessário para handoff.

`Confidence:` 85 · `Source:` Submitter direct (Q011, 2026-06-03) · `Status:` answered · `Disposition:` answered · `Hint:` Originador bem caracterizado: origem bottom-up (times de produto, engenharia e gestão), formalizada como aposta de produto comercial. Clears originator gate ≥ 70. Sponsor individual (pessoa) não nomeado — não bloqueia; pode ser confirmado se necessário para handoff. Supersede assumption conf 60 registrada anteriormente (Q008 intermediário, Rev 1).
...
| **Product Owners (PO)** | Conduzem ou participam das sessões; perdem rastreabilidade das estimativas passadas e contexto para priorização futura. |
...
| **Gestores de produto / Engenharia / Gestão** | Dependem das estimativas para planejamento; ficam sem visibilidade da evolução, sem dados consolidados e sem integração com ferramentas de gestão. |

**Segmentos de mercado comercial (organizações-alvo do SaaS):**

| Segmento | Caracterização |
|---|---|
| **Empresas de tecnologia** | Squads internos de desenvolvimento com backlog regular; mais alta frequência de sessões de estimativa. |
| **Consultorias de software** | Múltiplos times, múltiplos clientes; necessidade de rastreabilidade e histórico por projeto. |
| **Fábricas de software** | Alto volume de sessões; necessidade de padronização do processo entre times. |
| **Times de produto (product-led companies)** | Estimativa frequente ligada a ciclos de produto; necessidade de integração com ferramentas de gestão (Jira, Linear). |
| **Organizações Scrum/ágil** | Qualquer organização que adotou Scrum ou métodos ágeis com rituais de estimativa recorrentes. |

> **Nota de escopo:** personas e segmentos identificados e confirmados pelo Submitter (Q008 + Q010). **Sem dimensionamento de TAM**: quantos times, quantas organizações, nº de orgs no mercado endereçável — não quantificado. Dimensionamento de mercado (TAM) é item de discovery.
...
O impacto de negócio tem **duas camadas distintas**, com diferentes graus de validação.

---

### Camada A — Eficiência operacional por cliente (modelo estimado)

Baseado em aproximação do Submitter para equipes ágeis tradicionais (Q009):
...
O problema não é a existência dessas horas (estimar é necessário), mas o **desperdício por falta de estrutura**: ausência de histórico, repetição de discussões não documentadas, dificuldade de registrar decisões e fragmentação de ferramentas.

**Ganho estimado com redução de ineficiência:**
- Reduzindo 15–20 min de ineficiência por sessão, um squad economizaria horas-homem/ano por time (valor exato a confirmar — ver inconsistência aritmética abaixo).
...
> **Inconsistência aritmética a validar:** 15–20 min × ~48 sessões/ano × 8 pessoas = ~96–128 h-h/ano por squad. O Submitter citou "8–11 h-h/ano" — há discrepância (base do cálculo de economia a confirmar antes de usar o número em materiais de triagem). Esta inconsistência não bloqueia o portão, mas deve ser resolvida na Discovery.

---

### Camada B — Oportunidade comercial (produto SaaS) — NÃO VALIDADA

O PokerPlan está sendo concebido como **produto comercial SaaS** (Q010). O problema é amplamente distribuído entre equipes de tecnologia ágil — não exclusivo de um time ou empresa.
...
Sem resposta capturada. Nenhuma janela, prazo, competidor chegando ou custo de espera foi declarado pelo Submitter.

`Confidence:` 0 · `Source:` — · `Status:` open · `Disposition:` — · `Hint:` "Por que agora" não foi declarado. Perguntar ao Submitter: há um prazo, um competidor, um evento de mercado, ou um custo que cresce a cada mês sem solução? Sem urgência declarada, o triage tende a Backlog ou Discovery — não invalida a demanda, mas enfraquece a prioridade.
...
**Level:** Não declarado pelo Submitter — **Reason:** Sem prioridade relativa declarada. O documento sugere escalonamento de escopo implícito:
...
| 2 | Recurring / has volume? | **Yes (assumed)** | Submitter descreve recorrência observada coletivamente em times de produto, engenharia e gestão (Q011). Origem bottom-up — múltiplos times reclamaram. Porém sem dimensionamento (quantos times, frequência por semana) — recorrência plausível mas não quantificada. | Q008 conf 78; Q011 conf 85; premissas (e) e (h) |
...
| 4 | Technical & business impact? | **Medium (modeled, unvalidated commercially)** | Eficiência operacional modelada (Q009): 32 h-h/mês por squad, ganho por redução de ineficiência — assumption conf 70, libera o portão honestamente. Oportunidade comercial (Q010): produto SaaS com mercado endereçável declarado. Porém: (a) inconsistência aritmética no modelo de economia a resolver; (b) validação de aderência problema-mercado, TAM e willingness-to-pay explicitamente ausentes — item de Discovery. | Q009 conf 70 (assumption); Q010 conf 80; premissas (f)–(h) |
| 5 | Do urgency & impact justify *now*? | **Partial (impact modeled; urgency absent)** | Impacto operacional modeled a conf 70 — justifica investigação. Oportunidade comercial real mas não validada. Urgência: sem janela, prazo, competidor ou custo de espera declarados (conf 0, não-bloqueante). O próprio Submitter declarou que o objetivo é "validar aderência ao problema antes de evoluir para produto comercial" — o que alinha com Discovery, não com Product Ready imediato. | Urgency conf 0; impact conf 70 assumption; Q010 Submitter direct |
...
**Needed:** No (DRAFT) — O MVP V1 é uma extensão de UI/estado com regras de negócio bem definidas (RN-001–RN-010), sem pagamentos, sem multi-tenancy complexo, sem AI/runtime, sem integrações externas (integrações Jira/Linear ficam para V2). A premissa de "entrada sem autenticação" (MVP V1) simplifica a superfície de segurança. As suposições técnicas remanescentes (estado em tempo real para múltiplos participantes simultâneos — premissa a) são de viabilidade e pertencem à racionalização pelo Tech Lead, não a uma escalada arquitetural antes do congelamento de escopo. Pendente confirmação do owner.
...
|---|---|---|
| (a) Sessões são remotas e síncronas com participantes simultâneos conectados ao mesmo tempo | To validate — material para arquitetura de estado em tempo real; se for async, o modelo muda | Submitter |
| (b) Sem autenticação no MVP V1 — identificar-se por nome é suficiente para o caso de uso inicial | To validate — pode ser preferência de design, não restrição permanente; confirmar se há requisito de identidade futura | Submitter |
...
| (d) Integrações com Jira, Linear e similares ficam para V2 — MVP V1 não depende delas | Accepted — declarado explicitamente na especificação como V2; baixo risco de reversão | Submitter (confirmar escopo V2) |
| (e) O público-alvo são equipes ágeis que já praticam Planning Poker — não há onboarding de método novo | To validate — impacta posicionamento e copy do produto; se for público novo, onboarding muda | Submitter |
...
| (h) O problema é amplamente distribuído entre times ágeis (base de mercado suficientemente grande para sustentar um SaaS) | To validate — premissa de mercado; distribuição qualitativa observada pelo Submitter, mas sem dados de TAM/SAM/SOM | Discovery (market sizing) |
...
|---|---|---|
| RN-001: Sigilo do voto até a revelação — progresso exibe apenas contagem de votos, não os valores | Technical / Scope | Regra de negócio central do Planning Poker; violá-la destrói o propósito do método |
| RN-002: Voto alterável apenas com rodada aberta — sem voto retroativo após encerramento | Technical / Scope | Garante integridade da rodada |
| RN-003: Controle exclusivo do host para revelar votos | Scope | Host/facilitador é o único autorizado a revelar |
| RN-005: Controle exclusivo do host para encerrar rodadas | Scope | Idem — controle de fluxo da sessão |
| RN-006: Múltiplas rodadas por tarefa permitidas | Scope | Suporta re-votação após discussão |
| RN-007: Histórico completo de todas as rodadas | Technical / Scope | Rastreabilidade é um objetivo central do produto |
| RN-008: Sem voto retroativo após encerramento da rodada | Technical / Scope | Complementa RN-002 |
| RN-009: Controle exclusivo do host para cancelar rodadas | Scope | Consistência de controle do facilitador |
| RN-010: Estimativa final registrada separada dos votos individuais — consenso não precisa ser a média | Scope | Facilita o registro do valor acordado pelo grupo |
| Entrada sem autenticação (MVP V1): participantes identificados apenas por nome | Technical / Scope | Simplifica onboarding; confirmar se é restrição permanente ou só MVP |
| Preservar as regras do Planning Poker físico | Scope | Produto é uma versão digital fiel do método existente |

`Confidence:` 80 · `Source:` file:sources/pokerplan-spec.md §"Regras de Negócio" RN-001–RN-010, §"Fluxo Digital", §"Objetivo do Produto" (Q001) · `Status:` answered · `Disposition:` inferred · `Hint:` Confirmar com o Submitter quais são restrições rígidas vs. preferências de design — especialmente "sem autenticação" (pode ser só MVP V1, não permanente) e quais RNs são inegociáveis vs. configuráveis.
...
> Preenchido porque o triage draft propõe **Discovery**. O foco é validar aderência problema-mercado e resolver as premissas comerciais declaradas pelo Submitter — não coletar mais dados de captura (os campos bloqueantes estão todos acima do limiar). Pendente confirmação do owner antes de abrir formalmente.
>
...
| 5 | **Urgência / janela de entrada:** há uma janela de mercado, um evento ou um custo de espera que justifique agir agora vs. em 2–3 meses? (Competidores chegando? Demanda reprimida crescendo?) | Submitter | Análise de competidores existentes; pergunta direta nas entrevistas sobre tools atuais e satisfação | Simultâneo ao Q1 |

**Owner sugerido:** Submitter / time de produto.

**Time-box total:** 3 semanas (2026-06-03 → 2026-06-24). Os itens 1–3 podem ser cobertos em uma rodada de 5–10 entrevistas de 30–45 min cada. O item 4 pode ser paralelo (pesquisa desk). Se os itens 1–3 confirmarem aderência e willingness-to-pay, re-triage para **Product Ready** é imediato — sem nova rodada de intake completa.

**Critério de saída da Discovery (para re-triage):** (a) pelo menos 5 entrevistas realizadas com personas do segmento-alvo; (b) evidência qualitativa de que a dor é real e há disposição para pagar; (c) modelo de impacto operacional corrigido com dados reais; (d) premissas (f)–(h) de assumptions revisadas com base nas entrevistas.
...
- **Se Discovery (proposta atual — DRAFT):** executar o Discovery brief acima. Objetivo: validar aderência problema-mercado, willingness-to-pay e corrigir o modelo de impacto operacional. Time-box: 3 semanas. Ao fechar os critérios de saída definidos no brief, re-triage para **Product Ready** é imediato — sem nova rodada de intake completa.
- **Se o owner re-triar para Product Ready após a Discovery:** prosseguir para racionalização (Readiness Package). Todos os campos bloqueantes já estão acima do limiar; a racionalização focará nas premissas técnicas (a)–(e) e nas premissas comerciais confirmadas pela Discovery. Urgência pode ser re-avaliada se a Discovery revelar janela de mercado.
- **Se o owner re-triar para Backlog:** registrar rationale e notificar os originadores (times de produto, engenharia e gestão que levantaram a demanda coletivamente).
- **Se a Discovery revelar ausência de aderência:** registrar aprendizado, arquivar a demanda com evidência, e decidir se volta como versão reduzida (ferramenta interna) ou é descartada.

> ⚠️ **Ação imediata:** o owner deve (1) confirmar ou ajustar o triage draft acima, (2) atribuir-se como triador e registrar data de triage em Metadata, (3) abrir o Discovery brief com o Submitter/time de produto e agendar o início das entrevistas de descoberta. Esta demanda não está em Discovery por gaps de captura (todos fechados) — está em Discovery por ser o próximo passo honesto antes de comprometer capacidade de desenvolvimento em um mercado ainda não validado.
