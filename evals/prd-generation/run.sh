#!/usr/bin/env bash
# Eval runner for the hsb-teamwork:prd-generation skill (repo-level, dev/CI only).
...
# assemble_run <run_dir> <doc> -> outputs/prd-document.md + grading.json
assemble_run() {
  mkdir -p "$1/outputs"
  cp "$2" "$1/outputs/prd-document.md"
  python3 assertions.py "$1/outputs/prd-document.md" --grading-json "$1/grading.json" >/dev/null 2>&1 || true
}

echo "== Self-test: grading the golden =="
if g=$(grade golden/rbac.prd-document.md); then
  IFS=$'\t' read -r st rd bl <<<"$g"
  echo "golden: $st readiness=$rd blocking=$bl"
  echo "| golden (self-test) | fixture | $st | $rd | $bl | — |" >> "$SCORE"
fi
# Always lay out a viewer run for the golden so ./view.sh is demonstrable without
# the claude CLI (it appears under the Outputs tab). eval_id -1 keeps it an integer
# (the viewer sorts runs by eval_id and cannot compare a string id against the
# integer ids of the live cases); -1 sorts first.
GRUN="$RUNS/golden-selftest"; mkdir -p "$GRUN"
write_meta "$GRUN" -1 "Self-test: grade the committed golden PRD (golden/rbac.prd-document.md). No agent run — this populates the eval-viewer so it can be demonstrated without the claude CLI."
assemble_run "$GRUN" golden/rbac.prd-document.md
...
' | while IFS=$'\x1f' read -r id name seed prompt; do
  for mode in with_skill baseline; do
    ...
    # receives the leaked lines as instructions (running the wrong cases).
...
' "$OUT/trace.jsonl" > "$OUT/agent.log" 2>/dev/null || true
    # Fan-out verdict (did the skill actually orchestrate the merge?).
    python3 fanout.py "$OUT/trace.jsonl" > "$OUT/fanout.json" 2>/dev/null || echo '{}' > "$OUT/fanout.json"
    fan=$(fanout_cell "$OUT/fanout.json" "$mode")
    if [ -f "$OUT/prd-document.md" ]; then
      res=$(grade "$OUT/prd-document.md") || res=$'FAIL\t-\t-'
      IFS=$'\t' read -r st rd bl <<<"$res"
      write_meta "$OUT" "$id" "$p"
      assemble_run "$OUT" "$OUT/prd-document.md"
    else
      st="NO-OUTPUT"; rd="-"; bl="-"
    fi
...
if python3 ../eval-viewer/make_benchmark.py "$RUNS" --skill "hsb-teamwork:prd-generation" > "$RUNS/benchmark.json" 2>/dev/null; then
...
echo "showing 'FAIL (inline, 0 spawns)' means the orchestrator built the PRD itself"
echo "instead of delegating the merge — the regression this column exists to catch."
echo "baseline is the control and is expected to stay inline. Per-run detail:"
echo "<case>/<mode>/fanout.json (reconciler_present, inheritor_fanout_in_turn)."
echo "Next: have an LLM grade each with_skill/outputs/prd-document.md against rubric.md"
