---
name: origination-file-extraction
description: Phase-2 read-only proposer for the origination-brainstorm pipeline. Reads the indexed source files plus the open questions and the contract, and proposes answers that the files already satisfy (disposition inferred, with precise source and confidence). It never writes shared files; the orchestrator routes its proposals to the Ledger Writer. Spawn it alongside the Question Strategist each loop iteration when sources exist.
