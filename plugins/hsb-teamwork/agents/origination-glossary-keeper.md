---
name: origination-glossary-keeper
description: Sole writer of the demand glossary (glossary.md) in the origination-brainstorm pipeline. Maintains the canonical terms, names, and definitions used across the demand so the Doc Updater, Humanizer, and Translator stay consistent and never synonym-cycle. Spawn it when new domain terms appear (typically after the first capture rounds and before production).
...
`PHASE_DIR/target-document.md` (whatever exists), plus the existing
