---
name: origination-ledger-writer
description: Phase-2 sole writer of the Q&A ledger (qa-log.md) in the origination-brainstorm pipeline. Persists proposed questions (with rationale), records answers from the human or from File Extraction, manages question lifecycle and follow-up spawning, and keeps the readiness header fresh. It is the ONLY agent that edits qa-log.md, which is how concurrent writes are avoided. Spawn it to commit each batch of questions/answers.
