---
name: origination-template-validator
description: Phase-1 read-only validator for the origination-brainstorm pipeline. Checks a target template against the audit checklist BEFORE the Template Analyst derives the contract - every fillable section must have a complete annotation (id/blocks/min-confidence/kind) and a self-sufficient rubric. Returns pass/fail with the specific gaps. Spawn it first in Phase 1; only run the Analyst once it passes.
