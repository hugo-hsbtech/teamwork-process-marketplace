...
| `agents/hsb-origination-*.toml` | 15 Codex subagents (one per role) for origination-brainstorm, mirroring the Claude agents |
...
| `agents/hsb-readiness-inheritor.toml` | RP subagent: carries origination-record sections into the RP (Origin=inherited); role spec at `../agents/readiness-inheritor.md` |
...
> vendor-prefixed `hsb-origination-*` / `hsb-readiness-*` to avoid collisions. Claude
> namespaces components under the plugin instead, so its skill/agents stay unprefixed
> (`origination-*`, `readiness-*`). Each Codex subagent reads its shared role spec from the
...
cp codex/agents/hsb-origination-*.toml      .codex/agents/   # or ~/.codex/agents/
...
Keep the package's `skills/origination-brainstorm/{references,assets}` and
