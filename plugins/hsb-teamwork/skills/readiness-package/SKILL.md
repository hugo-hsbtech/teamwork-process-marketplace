...
| 1 | `hsb-source-indexer` | normalize the intake-record folder + extra files into `sources/` |
...
| 2 | `hsb-reconciler` | resolve conflicts (intake-said-X / PO-says-Y) (read-only) |
...
|---|---|---|
| 1 | `hsb-stage-inheritor` | read-only proposer — carries the upstream intake-record forward into the RP's inheritable sections, preserving confidence/source/disposition |
...
directory), `SESSION_DIR`, `TEMPLATE`, `DOC` (the target document's filename —
...
| `references/inheritance.md` | Intake-to-RP section mapping, what `hsb-stage-inheritor` preserves and does not do |
